...
- [ ] **Verificare sul vivo la beta.11 su XQUXDU**: (a) `sensor.garage_xquxdu_trasporto_attivo` deve mantenere lo stesso `entity_id` e storico dopo l'update (migrazione unique_id in `async_setup_entry`, verificare nei log `Migrated ... unique_id` o semplicemente che non compaia un'entità duplicata); (b) provare a cambiare `connection_type` dalle opzioni e confermare che il reload avvenga ancora correttamente (ora pianificato dall'update-listener, non più dal flow).
- [ ] **Promuovere 1.4.0 → stable** — la verifica dal vivo su XQUXDU (beta.10) è OK, manca solo il via libera per il cut stabile; aggiungere anche la verifica beta.11 sopra prima del cut.
