...
- **Cloud-only `ContractedPower` displayed `0.007 kW` for a 7 kW contract** — the cloud reports `contract_power` in kW (`"7"`), but the Number entity layer divides the raw value by 1000 to render kW (LAN format is W). Added a `_CLOUD_TO_LAN_MULTIPLIERS` table that multiplies `ContractedPower` by 1000 unconditionally during synthesis, so cloud `"7"` becomes `7000` in the synthetic /RealTimeData and the entity renders the expected `7.0 kW`.
