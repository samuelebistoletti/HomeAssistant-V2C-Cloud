...
class TestProbeLocalApi:
    ...
    def _hass(self, session: ClientSession) -> MagicMock:
        hass = MagicMock()
        ha_aiohttp = __import__(
            "homeassistant.helpers.aiohttp_client", fromlist=["async_get_clientsession"]
        )
    ...
    async def test_valid_private_ip_returns_device_id(self):
        ...
        with aioresponses() as m:
            m.get(
                "http://192.168.1.50/RealTimeData",
                status=200,
                body=payload,
                content_type="text/plain",
            )
            with patch(
                "custom_components.v2c_cloud.config_flow.aiohttp_client.async_get_clientsession",
